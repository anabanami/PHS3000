...
def Task_1(p_values, max_positions):
    ...
    # plt.legend()
    # spa.savefig("peak_position_vs_pressure.png")
    # plt.show()
    return P_0, u_P_0
...
def Task_6(total_events):
    ...
    plt.errorbar(
        p_values[:15], total_events[:15],
        xerr=0.5,yerr=500,color='tomato', 
        marker='None',linestyle='None', label="detected alphas"
    )
    # plt.grid(linestyle=':')
    plt.xlabel(r'$p$ / kPa')
...
def Task_7_8(f, x, y):
    ...
    pars, pcov = scipy.optimize.curve_fit(f, x, y)
    ...
    p = np.linspace(x[0], x[-2], 1024)
    fit = f(p, p_R, pα)

    plt.errorbar(
        x[:-2], y[:-2],
        xerr=0.5,yerr=0.01,color='tomato', 
        marker='None',linestyle='None', label="detected alphas"
    )
    plt.plot(
            p, fit, marker='None',
    ...
    plt.xlabel("p / kPa")
    plt.ylabel("Normalised number of detected particles")
    # plt.title(r'Number alpha particles as a function of pressure'))
    plt.legend()
    spa.savefig("alphas_vs_pressure.png")
    # plt.show()
